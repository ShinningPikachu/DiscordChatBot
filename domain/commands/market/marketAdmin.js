...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addSubcommand(sub =>
      sub
        ...
        .addNumberOption(opt =>
          opt.setName('price').setDescription('Listing price in coins').setRequired(true),
        ),
    )
    .addSubcommand(sub =>
      sub
    ...
    .addSubcommand(sub =>
      sub
        ...
        .addIntegerOption(opt =>
          opt.setName('id').setDescription('Listing ID to buy').setRequired(true),
        ),
    ),
  ...
  async execute(interaction) {
    ...
    if (sub === 'remove') {
      ...
      await interaction.reply({ content: `Removed listing #${id} (${item.name}).`, ephemeral: true });
      await updateMarketBoard(interaction);
      return;
    }
    ...
    if (sub === 'buy') {
      ...
      await bagBoardService
        ...
        .catch(error => console.error('Bag sync failed after market admin buy', error));

      await interaction.reply({
        content: `Admin bought #${id} (${item.name}) for ${item.price} coins. Credited seller <@${item.seller}>.`,
        ephemeral: true,
      });
      await updateMarketBoard(interaction);
      return;
  ...
  async autocomplete(interaction) {
    try {
      ...
      console.error('Error during /marketadmin autocomplete:', error);
    }
  },
};
